...
- 第 6 节：可选模块（按需）—— 多模态、记忆、API、RAGAS

如果赶时间，只看第 0-2 节也能跑起来。

## 项目结构说明

先搞清楚目录结构，后面所有路径都基于这个：

    dsh-web-RAG/                  ← 你 clone 下来的仓库根目录
    ├── rag-private-docs/         ← 项目实际内容
    │   ├── src/                  ← 源代码
    │   ├── docs/                 ← 放你的文档
    │   ├── example_lessons/      ← 迷你实验
    │   ├── memory/               ← 对话记忆数据（自动创建）
    │   ├── .venv/                ← 虚拟环境（创建后出现）
    │   ├── requirements.txt
    │   └── .env                  ← 你的 API Key 配置
...
**重要**：虚拟环境、依赖安装、索引、运行，**都在 `rag-private-docs/` 目录里操作**，不是仓库根目录。
...
- 推荐用 Homebrew：先装 Homebrew，然后 `brew install python@3.11`。
- 验证：`python3 --version`。
...
- Windows：去 https://git-scm.com/download/win 下载安装（一路默认，建议勾选 "Git from the command line"）
...
4. **⚠️ 重要警告**：DeepSeek 的密钥**仅在创建时完整显示一次**，关闭弹窗后就再也看不到了。**请务必立刻复制并粘贴到安全的地方**。
...
项目提供了一个安装脚本，自动完成虚拟环境创建、依赖安装、.env 准备：
...
1. 检查 Python 版本
2. 在 `rag-private-docs/` 下创建 `.venv` 虚拟环境
3. 用清华镜像装依赖
4. 装 pytest
5. 从 `.env.example` 复制出 `.env`

跑完后，你需要手动编辑 `rag-private-docs/.env` 填入 API Key。

**如果你喜欢手动操作**，跳过这步，直接看 0.6 和 0.7。

### 0.6 手动创建虚拟环境和装依赖

进入 `rag-private-docs/` 目录：
...
> **PowerShell 激活报错**：若提示"禁止运行脚本"，以管理员身份运行 PowerShell，执行 `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser`，输入 Y，再重新激活。
...
**Windows 用户特别提醒**：`unstructured` 库安装时可能需要 C++ 编译器。如果报错：

1. 下载安装 Microsoft C++ Build Tools：https://visualstudio.microsoft.com/visual-cpp-build-tools/
2. 安装时勾选 "C++ 生成工具" 和 "Windows 10 SDK"
3. 重新运行 pip install

如果不需要解析 `.doc` 文件，可以注释掉 requirements.txt 里的 `unstructured` 一行，避免编译麻烦。

### 0.7 配置 API Key

在 `rag-private-docs/` 目录下，复制模板：
...
> **安全提示**：`.env` 包含敏感信息，**不要提交到 Git**。`.gitignore` 已经忽略了它。
...
检查目录结构、关键文件、Python 语法、单元测试、Git 状态。

## 第 1 节：4 个迷你实验

这一节跑代码，帮你建立"AI 到底在做什么"的直觉。

**前提**：确保已经完成 0.7（配置好了 .env）。

进入实验目录：

    cd rag-private-docs/example_lessons
...
2. **温度参数的影响**：同一问题，不同风格的回答（温度低则稳定，温度高则"创造力"更强）
3. **幻觉演示**：在强制 prompt 下，AI 可能编造不存在的信息

观察重点：第三步展示了"无根据回答"的风险。RAG 的核心价值正是通过提供真实文本来约束这种编造。

### 实验 2：把文字变成数字

    python 02_embeddings.py

**首次运行会下载 BGE 模型**（bge-small-zh-v1.5，FP32 约 91MB）。下载慢的话参考 README 设置 HF_ENDPOINT 镜像。

跑通后会看到 5 句话两两之间的相似度矩阵。重点观察：

- "今天天气真好" 和 "今日阳光明媚" 相似度约 0.75（高）
- 和 "我午餐吃了红烧肉" 相似度约 0.2（低）
- "My favorite food is braised pork" 和 "红烧肉" 相似度约 0.55（跨语言语义仍有响应）
...
建一个 5 篇文档的小型向量库，然后用查询去搜索。

注意：纯向量检索对 "AI 怎么学" 这种语义查询很准，但对 "LangChain 怎么用" 这种含专有名词的查询，可能不如关键词匹配精准。这就引出了下一个实验。

### 实验 4：完整 RAG 流程

    python 04_mini_rag.py

对比"无 RAG 的 AI"和"有 RAG 的 AI"。

问同一个问题（"蓝海科技签订的合同金额"）：

- 无 RAG：如果 prompt 不加限制，AI 会回答 "我没有相关信息"；如果强制它回答，可能编一个数字。
...
**首次运行会再次下载 BGE 模型**（实验 2 已下载过会复用缓存）。
...
检查 6 项：Qdrant 集合大小、BM25、Dense、Hybrid、Reranker、Mini-evaluator。如果 Qdrant 点数异常，运行 `python indexer.py --force` 清理累积的旧向量。
...
把文档复制到 `rag-private-docs/docs/` 文件夹。
...
| .pdf | PDF 文档 | 无（默认解析器） |
| .docx | 新版 Word | 无 |
| .doc | 旧版 Word | 需要 `unstructured`，否则提示转 .docx |
| .pptx | PowerPoint | 需要 `python-pptx` |
| .xlsx | Excel | 需要 `openpyxl` |
| .csv | 逗号分隔 | 无 |
...
系统准不准，受几个关键参数影响。这些参数都在 `rag-private-docs/src/config.py` 里。
...
1. **AI 回答时上下文明显不够** → 调大 `CHUNK_SIZE` 到 600-800，同时适当调大 `CHUNK_OVERLAP` 到 100-120。
2. **AI 引用了完全不相关的内容** → 调小 `TOP_K` 到 3，减少噪声干扰。
3. **问题很具体（如合同编号 "HT-2024-012"）但系统找不到** → 检查 `retriever.py` 是否开启混合检索（默认开启）。调大 `TOP_K` 到 10，观察 Top-20 里是否有目标段落。
...
然后重新运行 `python indexer.py --force`。

**注意**：换模型后向量维度会变。如果从 bge-small（512 维）换成 bge-base（768 维），需要改 `config.py` 里的 `EMBEDDING_DIM`。
...
1. 在 `rag-private-docs/eval/` 目录下新建 `test_set.json`（目录不存在则手动创建）
2. 格式：
...
**注意**：镜像默认不含你的 `qdrant_data/` 索引，首次运行需要在容器里建索引，或者把本地的 `qdrant_data` 挂载进去：
...
最简方案：租一台云服务器（阿里云、腾讯云等），把代码传上去。
    ...
    # 在本地打包（排除虚拟环境和数据）
    scp -r rag-private-docs user@server:/opt/rag/

    # 在服务器上
    ssh user@server
    cd /opt/rag/rag-private-docs
...
项目内置几个可选功能，默认关闭，需要时在 `src/config.py` 里开启。

### 6.1 可视化流水线面板

独立于聊天界面的五 Tab 面板（数据源 / 解析 / 分块 / 索引 / 检索）：

    cd rag-private-docs/src
    streamlit run pipeline_app.py

浏览器会打开新页面。适合想可视化调参、看每个阶段效果的用户。
...
可接任何 OpenAI 兼容的 VLM（OpenAI、通义千问 VL、智谱 GLM-4V 等）。
...
记忆存在 `rag-private-docs/memory/memories.json`。
...
- 加多模态支持（读取 PDF 中的图表），参考 6.4 节
- 多用户系统，让家人朋友也能用

最重要的：**拿你自己的真实数据去试**。教程里的示例是给你跑的，真正好玩的是把你自己的笔记、合同、邮件丢进去，让 AI 帮你找答案。
